...
setwd("/work/users/c/l/clairez1/Paper1sims2/DIPP")
...
combine_datasets <- function(start_id, end_id) {
  ...
  if (length(dfs) == 0) {
    ...
    # DIPP
    beta1.dipp  <- results.all %>% filter(variable == "beta[1]" & method == "dipp")
    beta2.dipp  <- results.all %>% filter(variable == "beta[2]" & method == "dipp")
    beta3.dipp  <- results.all %>% filter(variable == "beta[3]" & method == "dipp")
    tau.dipp    <- results.all %>% filter(variable == "tau" & method == "dipp")
    xi.dipp  <- results.all %>% filter(variable == "xi" & method == "dipp")
    n0exch.dipp <- results.all %>% filter(variable == "n0exch" & method == "dipp")
    rESS        <- n0exch.dipp$mean * 0.5
    ...
    sim.summary.dipp <- data.frame(rbind(beta1.summary, beta2.summary, beta3.summary, tau.summary, xi.summary, n0exch.summary, rESS.summary))
    ...
    sim.summary.dipp$param <- c("beta1", "beta2", "beta3", "tau", "xi", "n0exch", "rESS")
...
saveRDS(compiled.results, file = "sim_summary_dipp.rds")
