...
grid_new <- rbind(grid_extra, grid_add)

set.seed(304)
# Seed for generating historical data
grid_new$hist.seed <- sample(seq_len(1000 * nrow(grid_new)), nrow(grid_new), replace = FALSE)

# Create 10 rows of each scenario to divide up sims
grid <- grid_new[rep(1:nrow(grid_new), each = 20),]
...
# -----------------------------------------------------------
generate_hist_for_scenario2 <- function(n0.id, q.id, prob.unexch.id,
...
# -----------------------------------------------------------
set.seed(404041)
results_list2 <- lapply(seq_len(nrow(grid_new)), function(i) {
  row <- grid_new[i, ]
  generate_hist_for_scenario2(
    n0.id          = row$n0,
    q.id           = row$q,
...
hist_df2 <- do.call(rbind, results_list2)

saveRDS(hist_df2, file = file.path(save.dir, "simdata_hist_add.rds"))
