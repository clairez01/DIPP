...
setwd("/work/users/c/l/clairez1/Paper1sims3/LEAP_model")
...
combine_datasets <- function(start_id, end_id) {
  ...
  if (length(dfs) == 0) {
    ...
    # Compute bias
    beta2.bias  <- mean(data$mean - beta[2])
    
    # Compute MSE
    beta2.MSE  <- mean( (data$mean - beta[2])^2 )
    
    # Compute coverage probability
    beta2.covprob  <- sum(data$X2.5. <= beta[2] & data$X97.5. >= beta[2]) / nrow(data)

    # Compute 95% credible interval width
    beta2.width  <- mean(data$X97.5. - data$X2.5.)

    beta2.summary  <- cbind(mean(data$mean), sd(data$mean), beta2.bias, beta2.MSE, beta2.covprob, beta2.width)
    
    sim.summary.leap <- as.data.frame(beta2.summary)
    sim.summary.leap$method <- "leap"
    sim.summary.leap$n0 <- scen$n0
    sim.summary.leap$q  <- scen$q
    sim.summary.leap$prob.unexch <- scen$prob.unexch
    sim.summary.leap$param <- c("beta2")
    colnames(sim.summary.leap) <- c("mean", "sd", "bias", "MSE", "Coverage Prob.", "CI Width", "method", "n0", "q", "prob.unexch", "parameter")
    
    # Store summarized sims
    sim.summary <- rbind(sim.summary, sim.summary.leap)
    return(sim.summary)
  }
...
setwd("/nas/longleaf/home/clairez1/Paper1_sims_revisions/compiled_sim_results")
saveRDS(compiled.results, file = "sim_summary_dipp_a0_0.8.rds")
