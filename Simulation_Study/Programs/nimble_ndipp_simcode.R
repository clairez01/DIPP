...
save.dir <- '/work/users/c/l/clairez1/Paper1sims2/NDIPP'
...
sampler_dipp_a0random_conjugate <- nimbleFunction(
  name = 'sampler_dipp_a0random_conjugate'  ## give name for sampler
...
## -----------------------------------------------------------------------------
## Compile normalized DIPP sampler 
## -----------------------------------------------------------------------------
...
init2 <- list(c0 = rbinom(n0.id, 1, c0_init_prob), xi = xi_init, a0 = a0_init)

model2    <- nimbleModel(dipp.a0random, constants = const2, data = data2, inits = init2)
cmodel2   <- compileNimble(model2)
mcmcConf2 <- configureMCMC(model2, monitors = c('beta', 'xi', 'tau', 'c0', 'n0exch', 'a0'))
mcmcConf2$removeSampler(c('beta', 'tau'))
mcmcConf2$addSampler(c('beta', 'tau'), 'sampler_dipp_a0random_conjugate')
...
for (i in 1:Nsims) {
  ...
  smpl.dipp    <- runMCMC(cmcmc2, niter = burnin + thin * niter, nburnin = burnin, thin = thin)
  summary.dipp <- summarize_draws(smpl.dipp, "mean", "sd", ~quantile(.x, probs = c(0.025, 0.975))) 
  results.dipp <- data.frame(summary.dipp %>% filter(grepl("beta", variable) | variable %in% c("xi", "tau", "n0exch", "a0")))
  results.dipp$method <- "ndipp"
  
  results <- results.dipp
