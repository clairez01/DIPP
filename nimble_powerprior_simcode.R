...
save.dir <- '/work/users/c/l/clairez1/Paper1sims2/PowerPrior'
...
sampler_pp_a0random_conjugate <- nimbleFunction(
  name = 'sampler_pp_a0random_conjugate'  ## give name for sampler
  , contains = sampler_BASE  ## doesn't change
  , setup = function(model, mvSaved, target, control) {  ## arguments always the same
    ## Place only things that are fixed here
    y0             <- model$y0
...
## -----------------------------------------------------------------------------
## Compile power prior sampler with a0 random
## -----------------------------------------------------------------------------

const2 <- list(n = n.id, n0 = n0.id, p = p)
data2  <- list(y = y, X = X, y0 = y0, X0 = X0
               , beta_init_mean = beta_init_mean
               , beta_init_prec = beta_init_prec
               , tau_init_shape = tau_init_shape
               , tau_init_rate = tau_init_rate
...
init2  <- list(beta = beta_init, tau = tau_init, a0 = a0_init)

model2    <- nimbleModel(pp.a0random, constants = const2, data = data2, inits = init2)
cmodel2   <- compileNimble(model2)
mcmcConf2 <- configureMCMC(cmodel2, monitors = c('beta', 'tau', 'a0'))
mcmcConf2$removeSampler(c('beta', 'tau'))
mcmcConf2$addSampler(c('beta', 'tau'), 'sampler_pp_a0random_conjugate')
...
for (i in 1:Nsims) {
  ...
  results.pp1$method <- "pp"
  
  smpl.pp2    <- runMCMC(cmcmc2, niter = burnin + thin * niter, nburnin = burnin, thin = thin)
  summary.pp2 <- summarize_draws(smpl.pp2, "mean", "sd", ~quantile(.x, probs = c(0.025, 0.975))) 
  results.pp2 <- data.frame(summary.pp2 %>% filter(grepl("beta", variable) | variable %in% c("a0","tau")))
  results.pp2$method <- "npp"
  
  results <- rbind(results.pp1, results.pp2)
